...
class Upsample(nn.Module):
    def __init__(self, in_channels, with_conv, both_d=True):
        super().__init__()
    ...
    def forward(self, x):
        x = torch.nn.functional.interpolate(x, scale_factor=2.0 if self.both_d else (1.0, 2.0), mode="nearest")
        if self.with_conv:
            x = self.conv(x)
        return x
    ...
    def __init__(self, in_channels, with_conv, both_d=True):
        ...
        if self.with_conv:
            ...
            self.conv = torch.nn.Conv2d(in_channels,
                                        in_channels,
                                        kernel_size=3,
                                        stride=2 if both_d else (1, 2),
                                        padding=0)

    def forward(self, x):
        if self.with_conv:
            pad = (0,1,0,1) if self.both_d else (0,1,1,1)
...
class Encoder(nn.Module):
    def __init__(self, *, ch, out_ch=3, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        for i_level in range(self.num_resolutions):
            ...
            down.attn = attn
            if i_level != self.num_resolutions-1:
                down.downsample = Downsample(block_in, resamp_with_conv, both_d=self.symmetric or (i_level % 2 == 0))
                curr_res = curr_res // 2
            self.down.append(down)
...
class Decoder(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        self.gumbel_sigmoid = GumbelSigmoid(hard=True, tau=1, pixelwise=True)
        ...
        for i_level in reversed(range(self.num_resolutions)):
            ...
            up.attn = attn
            if i_level != 0:
                up.upsample = Upsample(block_in, resamp_with_conv, both_d=self.symmetric or (i_level % 2 == 1))
                curr_res = curr_res * 2
            self.up.insert(0, up) # prepend to get consistent order
