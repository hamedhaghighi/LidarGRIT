...
__global__ void approxmatch(int b, int n, int m,
                            ...
                            scalar_t *__restrict__ match, scalar_t *temp) {
        ...
        for (int l0 = 0; l0 < m; l0 += Block) {
          ...
          if (k < n) {
            for (int l = 0; l < lend; l++) {
              ...
              scalar_t y2 = buf[l * 4 + 1];
              scalar_t z2 = buf[l * 4 + 2];
              scalar_t w = __expf(level * ((x2 - x1) * (x2 - x1) +
                                           (y2 - y1) * (y2 - y1) +
                                           (z2 - z1) * (z2 - z1))) *
                           rl * buf[l * 4 + 3];
...
// void approxmatchLauncher(int b,int n,int m,const scalar_t * xyz1,const
// scalar_t * xyz2,scalar_t * match,scalar_t * temp){
...
at::Tensor ApproxMatchForward(const at::Tensor xyz1, const at::Tensor xyz2) {
  ...
  AT_DISPATCH_FLOATING_TYPES(
      xyz1.scalar_type(), "ApproxMatchForward", ([&] {
        approxmatch<scalar_t>
            <<<32, 512>>>(b, n, m, xyz1.data<scalar_t>(), xyz2.data<scalar_t>(),
...
// void matchcostLauncher(int b,int n,int m,const scalar_t * xyz1,const scalar_t
// * xyz2,const scalar_t * match,scalar_t * out){
...
Output:
  cost: (B)
*/
at::Tensor MatchCostForward(const at::Tensor xyz1, const at::Tensor xyz2,
  ...
  AT_DISPATCH_FLOATING_TYPES(
      xyz1.scalar_type(), "MatchCostForward", ([&] {
        matchcost<scalar_t>
            <<<32, 512>>>(b, n, m, xyz1.data<scalar_t>(), xyz2.data<scalar_t>(),
...
// void matchcostgradLauncher(int b,int n,int m,const scalar_t * xyz1,const
// scalar_t * xyz2,const scalar_t * match,scalar_t * grad1,scalar_t * grad2){
...
std::vector<at::Tensor> MatchCostBackward(const at::Tensor grad_cost,
  ...
  AT_DISPATCH_FLOATING_TYPES(
      xyz1.scalar_type(), "MatchCostBackward", ([&] {
        matchcostgrad1<scalar_t>
            <<<32, 512>>>(b, n, m, grad_cost.data<scalar_t>(),
                          xyz1.data<scalar_t>(), xyz2.data<scalar_t>(),
                          match.data<scalar_t>(), grad1.data<scalar_t>());
        matchcostgrad2<scalar_t><<<dim3(32, 32), 256>>>(
            b, n, m, grad_cost.data<scalar_t>(), xyz1.data<scalar_t>(),
            xyz2.data<scalar_t>(), match.data<scalar_t>(),
            grad2.data<scalar_t>());
      }));
  // THCudaCheck(cudaGetLastError());
